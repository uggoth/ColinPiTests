module_name = 'test_15_M_wall_tracking_v02.py'
...
def check_straight():
    if driver.check_data_ready():
        ...
        right = (mm07+mm03)/2
        diff = left-right
        print ('diff:{:4}  mm15:{:4}  mm11:{:4}  mm07:{:4}  mm03:{:4}'.format(
            diff, mm15, mm11, mm07, mm03))
        return True, diff
    else:
        return False, 0
...
if wait_for_blue(blue_button, 1000):
...
print (my_pico.send_command(serial_no_string, command), '\n')
time.sleep(1)
serial_no += 1
serial_no_string = '{:04}'.format(serial_no)
my_pico.send_command(serial_no_string, 'STOP')
